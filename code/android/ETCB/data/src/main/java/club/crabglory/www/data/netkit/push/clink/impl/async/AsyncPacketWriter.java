package club.crabglory.www.data.netkit.push.clink.impl.async;
...
import club.crabglory.www.data.netkit.push.clink.frames.AbsReceiveFrame;
import club.crabglory.www.data.netkit.push.clink.frames.CancelReceiveFrame;
import club.crabglory.www.data.netkit.push.clink.frames.HeartbeatReceiveFrame;
import club.crabglory.www.data.netkit.push.clink.frames.ReceiveEntityFrame;
import club.crabglory.www.data.netkit.push.clink.frames.ReceiveFrameFactory;
import club.crabglory.www.data.netkit.push.clink.frames.ReceiveHeaderFrame;
...
class AsyncPacketWriter implements Closeable {
    ...
    /**
     * 消费IoArgs中的数据，将orgs组装成frame
     * @param args IoArgs
     */
    synchronized void consumeIoArgs(IoArgs args) {
        if (frameTemp == null) { // 一帧的长度是IoArgs的buffer大小
            Frame temp;
            do {
                // 还有未消费数据，则重复构建帧
                temp = buildNewFrame(args); // 要么为空帧，要么全部消费完
