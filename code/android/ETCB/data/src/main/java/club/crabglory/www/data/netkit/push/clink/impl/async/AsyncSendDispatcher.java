...
/**
 * 两种职责：
 * 1. 发送消息Packet，存在Queue中
 * 2. 等待SocketChannelAdapter中阻塞消费，也是拿到当前对象，调用IoArgs.IoArgsEventProcessor的方法进行消费
 */
public class AsyncSendDispatcher implements SendDispatcher,
    ...
    private void requestSend() {
        synchronized (isSending) {//状态锁
            if (isSending.get() || isClosed.get()) {
                return;
            }

            // 返回True代表当前有数据需要发送，将头包发送
            if (reader.requestTakePacket()) {
                try {
                    isSending.set(true);
                    boolean isSucceed = sender.postSendAsync();//用来注册
                    if (!isSucceed) {
                        isSending.set(true);
                    }
                } catch (IOException e) {
